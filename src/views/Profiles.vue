<template>
  <div class="wrapper">
      ...
      <p>
        <label for="size">Number of profiles </label>
        <input id="size" v-model="size" type="number" name="size" min="1">
      </p>

      <p>
        <input type="button" value="Search" @click="getProfiles">
      </p>
...
export default {
  ...
  methods: {
    getProfiles () {
      ...
      axios
        .post('https://search.torre.co/people/_search?size=' + this.size + '&aggregate=false&offset=0',
          {
            name: {
              term: this.name
            }
